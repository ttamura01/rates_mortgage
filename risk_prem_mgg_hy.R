...
long_data %>% 
  ...
  geom_hline(yintercept = mean_spread, color = "#0DE4C1") +
  geom_hline(yintercept = mean_sd, color = "#0DE4C1") +
  geom_hline(yintercept = mean_2sd, color = "#0DE4C1") +
...
avg_spread <- rates %>% 
  select(date, spread) %>% 
  left_join(., high_yld, by = "date") %>% 
  summarise(avg_mortgage_spread = mean(spread, na.rm = TRUE),
            avg_hy_spread = mean(high_yield, na.rm = TRUE)) 

avg_mortgage_spread <- avg_spread %>% pull(avg_mortgage_spread) %>% round(3)
